...
BOT_TOKEN="8026957316:AAHMgf9Qxb1tOt86UfJ4l4I3PEwJ-qDEO8s"
...
banner() {
    ...
    echo "            跨链桥自动化脚本            "
    echo "========================================================="
    echo "       关注 Twitter | 高效跨链，安全可靠！             "
    echo "       请按顺序安装配置，以免报错无法运行             "
    echo "       TG用户ID获取：@getmyid_bot                     "
    echo "       实时通知：@t3rntz_bot                          "
    echo "---------------------------------------------------------"
    echo "  支持网络：                                           "
    echo "    - Arbitrum Sepolia   - Optimism Sepolia            "
    echo "    - Base Sepolia       - Unichain Sepolia            "
    echo "---------------------------------------------------------"
    echo "  双向跨链：                                           "
    echo "    - ARB ↔ UNI    - OP ↔ UNI     - ARB ↔ OP           "
    echo "    - BASE ↔ OP    - ARB ↔ BASE   - UNI ↔ BASE         "
    echo "---------------------------------------------------------"
    echo "  单向跨链：                                           "
    echo "    - UNI → ARB    - UNI → OP      - UNI → BASE        "
...
# === 添加 Telegram ID ===
add_telegram_id() {
    echo -e "${CYAN}请输入 Telegram 用户 ID（纯数字，例如 7151449617）：${NC}"
